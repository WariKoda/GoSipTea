...
func runCommand(ctx context.Context, maxOutputBytes int, args ...string) ([]byte, error) {
	...
	cmd := exec.CommandContext(ctx, "wpctl", args...)
	cmd.Stdout = stdout
	cmd.Stderr = stderr
	...
	if stdout.overflow || stderr.overflow {
		return nil, fmt.Errorf("wpctl output exceeded %d bytes per stream", maxOutputBytes)
	}
	if err != nil {
		message := strings.TrimSpace(stderr.String())
		if message != "" {
			return nil, fmt.Errorf("wpctl: %w: %s", err, message)
		}
		return nil, fmt.Errorf("wpctl: %w", err)
	}
